...
def impl(context):
    assert reverse("student_view") in context.browser.current_url, "Assertion failed. {url} not in {current_url}".format(
        url=reverse("student_view"), current_url=context.browser.current_url)
    ...
    if not progress_verbs == "not started":
        ...
        for i in range(0, log.attempts):
            attempt_log, created = AttemptLog.objects.get_or_create(
                exercise_id=exercise, user=user, seed=i, timestamp=datetime.datetime.now())
...
def impl(context, exercise, learner, progress_text, progress_colour):
    ...
    assert cell.text == progress_text, "Progress text for {learner}, on {exercise} incorrect".format(
        learner=learner, exercise=exercise)
...
def impl(context):
    click_and_wait_for_id_to_appear(context, find_css_with_wait(
        context, "td.complete[value=subtraction_1]"), "details-panel-view")
