...
def get_next_recommendations(user, request):
    ...
    if current_subtopic:
        topic_tree_based_data = generate_recommendation_data()[current_subtopic]['related_subtopics'][
...
def get_group_recommendations(user):
    ...
    if recent_exercises:
        ...
        exercise_counts = ExerciseLog.objects.filter(user__in=user_list).values(
            "exercise_id").annotate(count=Count("exercise_id"))
...
def get_explore_recommendations(user, request):
    ...
    recent_subtopics = list(set([exercise_parents_table[ex]['subtopic_id']
...
def get_most_recent_exercises(user):
    ...
    exercises_by_user = ExerciseLog.objects.filter(user=user).order_by(
        "-latest_activity_timestamp").values_list("exercise_id", flat=True)
